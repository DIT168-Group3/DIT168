...
std::string V2VService::getIP(){
    ...
    close(fd);
    //display result
    char *s1 = inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr);
    std::string ip = s1;
    std::cout<<"Found the IP of the machine: "<<ip << "\n" <<std::endl;
    return ip;
}
